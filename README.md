...
Front end con flutter - multiplataforma
Incluye las mismas funcionalidades actuales:
Login contra POST /api/auth/login.
Sesión persistente con SharedPreferences.
Dashboard autenticado con módulos Chat y Productos.
Chat en tiempo real consumiendo Socket.IO del backend actual.
Gestión de productos:listar
crear
editar productos propios
eliminar productos propios
mostrar creador desde User.name

AppBar con formato NombreUsuario (N) usando el conteo de productos del usuario.
...
En una maquina con flutter ejecutamos:
cd flutter_frontend
flutter create .
flutter pub get
flutter run
